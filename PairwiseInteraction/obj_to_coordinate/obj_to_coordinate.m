...
% inputs: 
    fName = 'examples/46.obj'; %name of the obj file to read (I only considered cases with only one object, please triangularize the meshes, non-triangular mesh is not supported)
    lName = 'examples/46ligand3.obj';
    coarse_size = 0.667; %size of the "atoms" (length, unitless)
    box_size = 105;  %the volume to consider: box_size-by-box_size-by-box_size (length, unitless)
    ligand_density = 1; %ligands will be placed onto every meshes with this density (count*length-2, unitless)
    ligand_dist = 0.2; %ligand-to-surface distance (length, unitless)
% outputs:
...
% ligand_coordinate.txt, positions of ligands, same as above
    ...
    shape = readObj(fName);
    ligand = readObj(lName);
    object.vertices = shape.v;
    object.faces = shape.f.v;
    lobject.vertices = ligand.v;
    lobject.faces = ligand.f.v;
    units= place_atoms(object,coarse_size,box_size);
    ligands = place_ligand(lobject,ligand_density,ligand_dist);
    figure(1);clf;
    hold on
    ...
    zlim([-box_size/2,box_size/2])
    fileID = fopen('55_atom_coordinate.txt','w');
    fprintf(fileID,'%f %f %f\n',units');
    fclose(fileID);
    disp(['number of atoms: ', num2str(length(units))])
    fileID = fopen('55_ligand_coordinate.txt','w');
    fprintf(fileID,'%f %f %f\n',ligands');
    fclose(fileID);
...
ligands = [];
    for i = 1 :size(object.faces,1)
        ...
        x_ = n;
        y_ = l1/norm(l1);
        z_ = cross(x_,y_);  %area; can be rechecked with density
        z_ = z_/norm(z_);
